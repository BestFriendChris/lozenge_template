...
func (ct *ContentTokenizer) NextTokenCodeUntilOpenBraceLoz(in *input.Input) (*token.Token, error) {
	...
	if err != nil {
		...
		goCode = strings.TrimSuffix(goCode, "◊")
...
func (ct *ContentTokenizer) parseLozenge(in *input.Input) ([]*token.Token, error) {
	...
	singletonLoz := []*token.Token{token.NewToken(token.TTcontent, string(ct.loz))}
	r, found := in.Peek()
	if !found {
	...
	case '.':
		in.Shift(r)
		return ct.parseMacroIdentifier(in)
	case '^':
		in.Shift(r)
		r, found = in.Peek()
		if found && r == '{' {
...
func (ct *ContentTokenizer) parseMacroIdentifier(in *input.Input) (tokens []*token.Token, err error) {
	identifier := ct.readIdentifier(in)
	if len(identifier) == 0 {
		tok := token.NewToken(token.TTcontent, string(ct.loz))
		return []*token.Token{tok}, nil
	}
	in.UnshiftString(identifier)
